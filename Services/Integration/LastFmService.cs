...
public static class TagDenylist
{
    ...
    public static bool IsBlocked(string tag) => BlockedTags.Contains(tag);
}
    ...
    public async Task<LastFmSessionResult> GetSessionKeyAsync(string token)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                var msg = errDoc.RootElement.TryGetProperty("message", out var m) ? m.GetString() : "Authorization not yet granted";
...
public class LastFmService
{
    ...
    public async Task<LastFmTrackInfo?> GetTrackInfoAsync(string title, string artist)
    {
        ...
        try
        {
            ...
            {
                info.Tags = responseObj.Track.TopTags.TagList.Select(t => t.Name).Where(n => !string.IsNullOrEmpty(n) && !TagDenylist.IsBlocked(n)).Take(5).ToList();
            }
    ...
    public async Task<bool> ScrobbleAsync(string title, string artist, DateTime playedAt)
    {
        ...
        if (string.IsNullOrWhiteSpace(title) || string.IsNullOrWhiteSpace(artist) || artist.Equals("Unknown Artist", StringComparison.OrdinalIgnoreCase) || artist.Equals("Unknown", StringComparison.OrdinalIgnoreCase))
            return false;
